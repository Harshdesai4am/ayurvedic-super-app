import cartReducer, { addToCart, removeFromCart, updateQuantity, applyCoupon } from '../src/modules/shop/store/cartSlice';
import { Product } from '../src/modules/shop/types/shopTypes';
...
describe('Cart Reducer Tests', () => {
  it('should add item to cart', () => {
    const initialState = { items: [], couponCode: null, discountPercentage: 0 };
  ...
  it('should increment quantity when adding duplicate product', () => {
    const initialState = { items: [{ product: mockProduct, quantity: 1 }], couponCode: null, discountPercentage: 0 };
    const nextState = cartReducer(initialState, addToCart(mockProduct));
